...
namespace CircularQueues_CSharp
{
    class Queues
    {
        ...
        public void insert(int element)
        {
            ...
            {
                /* If REAR is at the last position of the array, then then value of
                 * REAR is set to 0 that corresponds to the first position of the array.*/
                if (REAR == max - 1)
                    REAR = 0;
                else
                    /* If REAR is not at the last position, the its value is 
                                        incremented by one.*/
                    REAR = REAR + 1;
            }
        ...
        public void display()
        {
            ...
            {
                /* traverse the queue till the last element present in a array. */
            ...
            else
            {
                ...
                FRONT_position = 0;
                /* traverse the array till the last element presents in the queue. */
                while (FRONT_position <= REAR_position)
                {
                    Console.Write(queue_array[FRONT_position] + "   ");
                    FRONT_position++;
